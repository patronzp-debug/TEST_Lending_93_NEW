...
## Етап 1: Архітектура та MVP (Завершено)
...
## Етап 2: QA та виправлення багів (Bug Tracker)
- [x] **Баг №1 [Виправлено]:** Зламаний стиль при автозаповненні (Autofill) форми. Інпути заливалися білим фоном браузера, що порушувало темну тему. Виправлено через додавання перевизначень `input:-webkit-autofill` у `globals.css` з `-webkit-box-shadow` в колір `#080808` та білим `-webkit-text-fill-color`.
- [x] **Баг №2 [Виправлено]:** Горизонтальний скролл (Overflow) на мобільних пристроях. Виникав через перекриття `overflow-x: clip` у `globals.css` застарілим класом `overflow-x-hidden` на `body` у `layout.tsx` (який перетворювався на `overflow-x: hidden` та ламав Lenis / спричиняв горизонтальний зсув). Виправлено шляхом заміни класу на `overflow-x-clip` у `layout.tsx` та додаванням класу `overflow-x-clip` до головного `<main>` у `page.tsx`.
